...
def rename_variable(v: str) -> str:
    """
    Rename regions to be more human readable
    """
    return (
...
def extract_gwp_context(v: str) -> str | None:
    """Extract GWP name from variable"""
...
def rename_regions(d: scmdata.ScmRun) -> scmdata.ScmRun:
    """
    Extract a country name from the region code
    """
...
for region in data.get_unique_meta("region"):
    if pycountry.countries.get(alpha_3=region) is None:
