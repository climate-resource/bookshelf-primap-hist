...
@pytest.fixture
def primap_book(local_bookshelf: os.PathLike, monkeypatch) -> LocalBook:
    monkeypatch.setenv(
        "BOOKSHELF_NOTEBOOK_DIRECTORY", Path(__file__).parent.parent / "src"
    )
...
@pytest.mark.xfail(
    reason=(
        "scmdata reading bug: https://github.com/climate-resource/bookshelf/issues/87"
    )
)
